...
- current exact progress reached during this tranche: `65 / 65` windows
...
## Follow-on tranche (verified continuation to 62 / 65)

The next bounded one-window continuation confirmed that predicted ordinary
advance:

- progress `62 / 65`
- elapsed approximately `763.19s`
- `next_window_index = 62`
- `state_window_index = 61`
- lead state `0`

This lifts the observed ordinary-step high-water mark from approximately
`733.21s` to approximately `763.19s`. Because the resulting lead state is now
zero, the next pending quartic target-side step is predicted to be a rebase
case.

## Follow-on tranche (verified continuation to 63 / 65)

The next bounded one-window continuation confirmed that predicted rebase case:

- progress `63 / 65`
- elapsed approximately `1177.09s`
- `next_window_index = 63`
- `state_window_index = 62`
- lead state nonzero

This lands below the current observed rebase high-water mark of approximately
`1225.92s`. Because the resulting lead state is now nonzero, the next pending
quartic target-side step is predicted to be an ordinary advance.

## Follow-on tranche (verified continuation to 64 / 65)

The next bounded one-window continuation confirmed that predicted ordinary
advance:

- progress `64 / 65`
- elapsed approximately `822.92s`
- `next_window_index = 64`
- `state_window_index = 63`
- lead state `0`

This lifts the observed ordinary-step high-water mark from approximately
`763.19s` to approximately `822.92s`. Because the resulting lead state is now
zero, the final pending quartic target-side step is predicted to be a rebase
case.

## Follow-on tranche (verified completion to 65 / 65)

The final bounded one-window continuation confirmed that predicted rebase case
and wrote a complete partial target-side cache:

- progress `65 / 65`
- `next_window_index = 65`
- post-retry `state_window_index = 65`
- partial-cache status `complete`
- profile count `65`
- lead state nonzero

The first completion run did not print its `perf_counter` elapsed line because
it failed after the complete partial-cache write while materializing the final
sequence cache:

- exception class: `ValueError`
- failure mode: Python's integer string conversion guard rejected a `16529`
  digit numerator while deserializing cached profile fractions
- direct cache inspection after the failure showed the partial cache was already
  complete at `65 / 65`

A follow-up materialization run with `sys.set_int_max_str_digits(0)` returned
the clean completion object:

- `status = complete`
- `completed_window_count = 65`
- `total_window_count = 65`
- `next_window_index = 65`
- `coordinate_count = 15`
- final-cache materialization elapsed approximately `592.86s`
- final sequence cache: `data/cache/bz_phase2_sym4_sixteen_window_target_sequence_cache.json`
- final sequence cache size: `131681112` bytes

The final rebase is therefore known to complete the cached quartic target side,
but its exact one-window `perf_counter` elapsed value is unavailable because the
post-write digit-limit exception prevented the wrapper from printing it. Live
wall-clock observation showed that the final rebase exceeded the previous
`1225.92s` rebase high-water before that post-write failure.

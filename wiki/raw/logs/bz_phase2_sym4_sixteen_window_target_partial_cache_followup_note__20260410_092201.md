...
- current exact progress reached during this tranche: `7 / 65` windows
...
The current cached state after the sixth completed window again had `coordinates[0] = 0`, so the next bounded resume required another rebase. After the seventh completed window, the cached lead is nonzero again. Through the first seven completed windows, the quartic target-side cache has therefore continued to exhibit an alternating pattern between nonsingular ordinary advances and singular-pivot recovery steps.

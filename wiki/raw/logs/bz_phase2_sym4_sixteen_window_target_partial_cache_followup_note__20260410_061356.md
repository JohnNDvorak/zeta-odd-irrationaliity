...
- current exact progress reached during this tranche: `6 / 65` windows
...
The current cached state after the sixth completed window again has `coordinates[0] = 0`, so the next bounded resume would require another rebase. Through the first six completed windows, the quartic target-side cache has therefore exhibited an alternating pattern between nonsingular ordinary advances and singular-pivot recovery steps.

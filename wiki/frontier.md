...
- Current quartic engineering subfrontier: the target-side cached continuation is tracked in
  [[sym4-sixteen-window-target-partial-cache-progress]], with persisted exact progress through `7 / 65` windows.
...
- The attempted Sym^4 sixteen-window continuation is currently an engineering blocker, but the target-side cache path is now real:
  ...
  - the target side remains too expensive to bank as a full object:
    ...
    - cached exact progress has now reached `7 / 65` windows
    - ordinary resumed advances are now about `84.06` and `93.39` seconds
    - singular-pivot rebases are now about `140.81`, `154.61`, and `188.13` seconds
  - the cache path is now resilient against the first discovered singular rolling pivot:
    ...
    - after the seventh completed window the cached lead is nonzero again, so the observed alternating pattern still holds

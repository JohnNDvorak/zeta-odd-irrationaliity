...
Current live frontier: frozen exact-side obstruction through degree 106, plus the seven-window normalized Plücker nonlinear frontier, with the first overdetermined constant-matrix ladder already closed through order 2.
...
- Current nonlinear frontier: the [[seven-window-normalized-plucker-object]] is now a repo-native exact paired object on
  `n=1..74`, with coordinate count `34`.

## Active interpretation

- The old exact lane is [[exact-side-frozen-frontier|frozen]], not active.
- The strongest live object is now the seven-window normalized Plücker invariant, not the six-window object and not any quotient of either.
- The seven-window object already closes its first bounded nonlocal matrix family:
  - source-side orders `1` and `2` are inconsistent mod `1013`
  - target-side orders `1` and `2` are inconsistent mod `1447`
  - order `3` is the first underdetermined case, so further order escalation on the same ladder would move from obstruction screening into interpolation territory
- The six-window object remains valuable as the predecessor frontier, and it already closed:
...
- The next defensible move is now either a genuinely different nonlocal family on the seven-window object, or another nonlinear invariant family beyond the current normalized Plücker layer. Another cheap local, scalar, quotient, or order-escalated constant matrix family is not justified.

## Related pages

- [[exact-side-frozen-frontier]]
- [[seven-window-normalized-plucker-object]]
- [[seven-window-normalized-plucker-frontier]]
- [[seven-window-normalized-plucker-matrix-recurrence-screen]]

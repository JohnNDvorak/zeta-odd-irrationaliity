...
- Current quartic engineering subfrontier: the target-side cached continuation is tracked in
  [[sym4-sixteen-window-target-partial-cache-progress]], with persisted exact progress through `18 / 65` windows.
...
- The attempted Sym^4 sixteen-window continuation is currently an engineering blocker, but the target-side cache path is now real:
  ...
  - the target side remains too expensive to bank as a full object:
    ...
    - cached exact progress has now reached `18 / 65` windows
    - ordinary resumed advances are now about `84.06`, `93.39`, `111.81`, `124.94`, `136.17`, `160.05`, `166.79`, and `191.74` seconds
    - singular-pivot rebases are now about `140.81`, `154.61`, `188.13`, `221.46`, `238.43`, `254.55`, `265.43`, and `306.43` seconds
  ...
  - live timing therefore narrows the quartic wall further:
    ...
    - the latest rebase rose to about `306.43` seconds, which is the first clear break above the prior rebase high-water mark
    - the latest ordinary step rose to about `191.74` seconds, which is the first clear break above the prior ordinary-step high-water mark

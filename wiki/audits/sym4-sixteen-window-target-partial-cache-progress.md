...
- Current banked progress in the latest raw follow-up note: `30 / 65` exact windows
...
- Through the first thirty completed windows, the observed cache pattern still alternates:
  - rebase step
  - ordinary step
  - rebase step
  - ordinary step
- After the thirtieth completed window, the cached lead is zero again, so the next quartic step is predicted to be a rebase.

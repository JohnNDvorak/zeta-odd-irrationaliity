...
- Current banked progress in the latest raw follow-up note: `21 / 65` exact windows
...
- Through the first twenty-one completed windows, the observed cache pattern still alternates:
  - rebase step
  - ordinary step
  - rebase step
  - ordinary step
- After the twenty-first completed window, the cached lead is nonzero again, so the next quartic step is predicted to be an ordinary advance.

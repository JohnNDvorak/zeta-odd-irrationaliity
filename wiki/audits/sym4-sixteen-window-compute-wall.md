...
- exact cached progress reached `59 / 65` windows
...
- the current persisted point is now `59 / 65`, with `state_window_index = 58`, `next_window_index = 59`, and nonzero lead after the latest rebase advance
...
- the persisted path through fifty-nine completed windows is consistent with an alternating rebase / ordinary-step pattern rather than a one-off singularity

...
from .symmetric_dual_baseline_sym2_eight_window_probe import (
    build_sym2_eight_window_probe,
    build_sym2_eight_window_sequences,
)

SYM2_EIGHT_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym2_eight_window_affine_matrix_recurrence_screen.md"
)
SYM2_EIGHT_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym2_eight_window_affine_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class Sym2EightWindowAffineMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class Sym2EightWindowAffineMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[Sym2EightWindowAffineMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_affine_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> Sym2EightWindowAffineMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return Sym2EightWindowAffineMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_sym2_eight_window_affine_matrix_recurrence_screen() -> (
    Sym2EightWindowAffineMatrixRecurrenceScreen
):
    probe = build_sym2_eight_window_probe()
    source, target = build_sym2_eight_window_sequences()

    results = []
    for recurrence_order in range(1, 3):
        results.append(
    ...
    return Sym2EightWindowAffineMatrixRecurrenceScreen(
        screen_id="bz_phase2_sym2_eight_window_affine_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "sym2_eight_window_affine_matrix_recurrence_requires_exact_followup"
            if any_open
        ...
        recommendation=(
            ...
            if any_open
            else "Do not keep increasing affine matrix recurrence order mechanically. Order 3 is the first underdetermined case on this lifted object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_sym2_eight_window_affine_matrix_recurrence_screen() -> str:
...
def write_sym2_eight_window_affine_matrix_recurrence_screen_report(
    output_path: str | Path = SYM2_EIGHT_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym2_eight_window_affine_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_sym2_eight_window_affine_matrix_recurrence_screen_json(
    output_path: str | Path = SYM2_EIGHT_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_sym2_eight_window_affine_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

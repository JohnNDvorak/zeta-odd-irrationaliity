...
SYM2_SEVEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym2_seven_window_affine_matrix_recurrence_screen.md"
)
SYM2_SEVEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym2_seven_window_affine_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class Sym2SevenWindowAffineMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class Sym2SevenWindowAffineMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[Sym2SevenWindowAffineMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_affine_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> Sym2SevenWindowAffineMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return Sym2SevenWindowAffineMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_sym2_seven_window_affine_matrix_recurrence_screen() -> (
    Sym2SevenWindowAffineMatrixRecurrenceScreen
):
    ...
    return Sym2SevenWindowAffineMatrixRecurrenceScreen(
        screen_id="bz_phase2_sym2_seven_window_affine_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "sym2_seven_window_affine_matrix_recurrence_requires_exact_followup"
            if any_open
            else "low_order_affine_matrix_recurrence_exhausted_through_order_10"
        ),
        ...
        recommendation=(
            ...
            if any_open
            else "Do not keep increasing affine matrix recurrence order mechanically. Order 11 is the first underdetermined case on this lifted object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_sym2_seven_window_affine_matrix_recurrence_screen() -> str:
...
def write_sym2_seven_window_affine_matrix_recurrence_screen_report(
    output_path: str | Path = SYM2_SEVEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym2_seven_window_affine_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_sym2_seven_window_affine_matrix_recurrence_screen_json(
    output_path: str | Path = SYM2_SEVEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_sym2_seven_window_affine_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

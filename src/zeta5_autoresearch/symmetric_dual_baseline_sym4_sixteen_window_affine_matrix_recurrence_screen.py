...
SYM4_SIXTEEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym4_sixteen_window_affine_matrix_recurrence_screen.md"
)
SYM4_SIXTEEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym4_sixteen_window_affine_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class Sym4SixteenWindowAffineMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class Sym4SixteenWindowAffineMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[Sym4SixteenWindowAffineMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_affine_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> Sym4SixteenWindowAffineMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return Sym4SixteenWindowAffineMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_sym4_sixteen_window_affine_matrix_recurrence_screen() -> (
    Sym4SixteenWindowAffineMatrixRecurrenceScreen
):
    probe = build_sym4_sixteen_window_probe()
    source, target = build_sym4_sixteen_window_sequences()

    results = []
    for recurrence_order in range(1, 4):
        results.append(
    ...
    return Sym4SixteenWindowAffineMatrixRecurrenceScreen(
        screen_id="bz_phase2_sym4_sixteen_window_affine_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "sym4_sixteen_window_affine_matrix_recurrence_requires_exact_followup"
            if any_open
            else "low_order_affine_matrix_recurrence_exhausted_through_order_3"
        ),
        source_boundary=probe.source_boundary,
        recommendation=(
            "A modular obstruction was not found on at least one side/order; exact follow-up is required before banking the lifted affine matrix ladder as exhausted."
            if any_open
            else "Do not keep increasing affine matrix recurrence order mechanically. Order 4 is already non-overdetermined on this lifted object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_sym4_sixteen_window_affine_matrix_recurrence_screen() -> str:
    screen = build_sym4_sixteen_window_affine_matrix_recurrence_screen()
    lines = [
...
def write_sym4_sixteen_window_affine_matrix_recurrence_screen_report(
    output_path: str | Path = SYM4_SIXTEEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym4_sixteen_window_affine_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_sym4_sixteen_window_affine_matrix_recurrence_screen_json(
    output_path: str | Path = SYM4_SIXTEEN_WINDOW_AFFINE_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_sym4_sixteen_window_affine_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    write_sym4_sixteen_window_affine_matrix_recurrence_screen_report()
    write_sym4_sixteen_window_affine_matrix_recurrence_screen_json()

...
from .symmetric_dual_baseline_six_window_plucker_object_spec import (
...
SIX_WINDOW_NORMALIZED_PLUCKER_PROBE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_six_window_normalized_plucker_probe.md"
)
SIX_WINDOW_NORMALIZED_PLUCKER_PROBE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_six_window_normalized_plucker_probe.json"
)
...
class SixWindowNormalizedPluckerSample:
    n: int
...
@dataclass(frozen=True)
class SixWindowNormalizedPluckerProbe:
    probe_id: str
    source_spec_id: str
    ...
    source_boundary: str
    recommendation: str
    samples: tuple[SixWindowNormalizedPluckerSample, ...]


def build_six_window_normalized_plucker_sequences() -> tuple[tuple[PluckerVector, ...], tuple[PluckerVector, ...]]:
    ...
    source = build_normalized_window_plucker_vectors(source_vectors, window_size=6)
    target = build_normalized_window_plucker_vectors(target_vectors, window_size=6)
...
def build_six_window_normalized_plucker_probe() -> SixWindowNormalizedPluckerProbe:
    ...
    source_hash = compute_sequence_hash(
        provisional_signature={"kind": "six_window_plucker_source", "terms": source_payload}
    )
    target_hash = compute_sequence_hash(
        provisional_signature={"kind": "six_window_plucker_target", "terms": target_payload}
    )
    paired_hash = compute_sequence_hash(
        provisional_signature={
            "kind": "six_window_plucker_transfer_object",
    ...
    samples = tuple(
        SixWindowNormalizedPluckerSample(
            n=n,
    ...
    return SixWindowNormalizedPluckerProbe(
        probe_id="bz_phase2_six_window_normalized_plucker_probe",
...
def render_six_window_normalized_plucker_probe() -> str:
    probe = build_six_window_normalized_plucker_probe()
    lines = [
        "# Phase 2 six-window normalized Plucker probe",
        "",
        f"- Probe id: `{probe.probe_id}`",
        f"- Source spec: `{SIX_WINDOW_NORMALIZED_PLUCKER_OBJECT_SPEC_REPORT_PATH}`",
...
def write_six_window_normalized_plucker_probe_report(
    output_path: str | Path = SIX_WINDOW_NORMALIZED_PLUCKER_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_six_window_normalized_plucker_probe(), encoding="utf-8")
    return output


def write_six_window_normalized_plucker_probe_json(
    output_path: str | Path = SIX_WINDOW_NORMALIZED_PLUCKER_PROBE_JSON_PATH,
) -> Path:
    probe = build_six_window_normalized_plucker_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

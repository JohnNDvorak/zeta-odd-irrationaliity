...
SYM2_EIGHT_WINDOW_OBJECT_SPEC_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym2_eight_window_object_spec.md"
)
SYM2_EIGHT_WINDOW_OBJECT_SPEC_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym2_eight_window_object_spec.json"
)


@dataclass(frozen=True)
class Sym2EightWindowPacketRole:
    packet_id: str
    family: str
    object_kind: str
...
@dataclass(frozen=True)
class Sym2EightWindowObjectSpec:
    spec_id: str
    object_label: str
    object_kind: str
    source_packet: Sym2EightWindowPacketRole
    target_packet: Sym2EightWindowPacketRole
...
def build_sym2_eight_window_object_spec() -> Sym2EightWindowObjectSpec:
    return Sym2EightWindowObjectSpec(
        spec_id="bz_phase2_sym2_eight_window_object_spec",
        object_label="Sym^2-lifted eight-window normalized maximal-minor object",
        ...
        invariant_definition=(
            ...
            "For each eight-term lifted window, form the normalized maximal-minor coordinate vector in `Gr(6,8)` by "
            "dividing all `6x6` minors by the pivot minor of the first six columns and omitting that pivot coordinate."
        ),
        rationale=(
            "The `Sym^2`-lifted seven-window object is now banked, and both its homogeneous and affine low-order matrix ladders are exhausted through order `10`. "
            "The strongest continuation inside the same lifted invariant family is therefore to widen the window once, because the resulting object still preserves "
            "an overdetermined matrix ladder through order `2` before interpolation takes over."
        ),
        source_boundary=(
...
def render_sym2_eight_window_object_spec() -> str:
...
def write_sym2_eight_window_object_spec_report(
    output_path: str | Path = SYM2_EIGHT_WINDOW_OBJECT_SPEC_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym2_eight_window_object_spec(), encoding="utf-8")
    return output


def write_sym2_eight_window_object_spec_json(
    output_path: str | Path = SYM2_EIGHT_WINDOW_OBJECT_SPEC_JSON_PATH,
) -> Path:
    spec = build_sym2_eight_window_object_spec()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

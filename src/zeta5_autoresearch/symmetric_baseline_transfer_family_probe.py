...
from .symmetric_baseline_transfer_post_probe_decision_gate import (
    SYMMETRIC_BASELINE_TRANSFER_POST_PROBE_DECISION_GATE_REPORT_PATH,
...
SYMMETRIC_BASELINE_TRANSFER_FAMILY_PROBE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_symmetric_baseline_transfer_family_probe.md"
)
SYMMETRIC_BASELINE_TRANSFER_FAMILY_PROBE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_symmetric_baseline_transfer_family_probe.json"
)


@dataclass(frozen=True)
class SymmetricBaselineTransferFamilyResult:
    family_id: str
...
@dataclass(frozen=True)
class SymmetricBaselineTransferFamilyProbe:
    probe_id: str
    source_gate_id: str
    shared_window_start: int
    shared_window_end: int
    overall_verdict: str
    family_results: tuple[SymmetricBaselineTransferFamilyResult, ...]
    source_boundary: str
    recommendation: str


def build_symmetric_baseline_transfer_family_probe() -> SymmetricBaselineTransferFamilyProbe:
    gate = build_symmetric_baseline_transfer_post_probe_decision_gate()
    source_packet = build_symmetric_linear_forms_exact_packet(max_n=80)
    source_vectors = tuple(
        (source_packet.scaled_q_terms[i], source_packet.scaled_p_terms[i], source_packet.scaled_phat_terms[i])
        for i in range(80)
    )
    ...
    target_vectors = tuple((constant_terms[i], zeta3_terms[i], zeta5_terms[i]) for i in range(80))
    ...
    return SymmetricBaselineTransferFamilyProbe(
        probe_id="bz_phase2_symmetric_baseline_transfer_family_probe",
        source_gate_id=gate.gate_id,
        shared_window_start=1,
        shared_window_end=80,
        overall_verdict=(
            "low_complexity_symmetric_baseline_transfer_has_winner"
            if winner_exists
            else "low_complexity_symmetric_baseline_transfer_exhausted"
        ),
...
def render_symmetric_baseline_transfer_family_probe() -> str:
    ...
    lines = [
        ...
        f"- Source gate: `{SYMMETRIC_BASELINE_TRANSFER_POST_PROBE_DECISION_GATE_REPORT_PATH}`",
...
def write_symmetric_baseline_transfer_family_probe_report(
    output_path: str | Path = SYMMETRIC_BASELINE_TRANSFER_FAMILY_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_symmetric_baseline_transfer_family_probe(), encoding="utf-8")
    return output


def write_symmetric_baseline_transfer_family_probe_json(
    output_path: str | Path = SYMMETRIC_BASELINE_TRANSFER_FAMILY_PROBE_JSON_PATH,
) -> Path:
    probe = build_symmetric_baseline_transfer_family_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
) -> SymmetricBaselineTransferFamilyResult:
    rows = []
    for n in range(3):
...
def _finish_family_result(
    *,
    ...
    return SymmetricBaselineTransferFamilyResult(
        family_id=family_id,
        ...
        transformed_residual_hash=compute_sequence_hash(
            provisional_signature={
                "kind": "symmetric_baseline_transfer_family",
                "family_id": family_id,
                "terms": [
...
def main() -> None:
    write_symmetric_baseline_transfer_family_probe_report()
    write_symmetric_baseline_transfer_family_probe_json()

...
from .symmetric_dual_baseline_sym3_eleven_window_matrix_recurrence_screen import (
    SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
    build_sym3_eleven_window_matrix_recurrence_screen,
)

SYM3_ELEVEN_WINDOW_DECISION_GATE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym3_eleven_window_decision_gate.md"
)
SYM3_ELEVEN_WINDOW_DECISION_GATE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym3_eleven_window_decision_gate.json"
)


@dataclass(frozen=True)
class Sym3ElevenWindowStatus:
    packet_side: str
...
@dataclass(frozen=True)
class Sym3ElevenWindowDecisionGate:
    gate_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    statuses: tuple[Sym3ElevenWindowStatus, ...]
    outcome: str
    rationale: str
    next_step: str
...
def build_sym3_eleven_window_decision_gate() -> Sym3ElevenWindowDecisionGate:
    screen = build_sym3_eleven_window_matrix_recurrence_screen()
    if screen.overall_verdict == "sym3_eleven_window_matrix_recurrence_requires_exact_followup":
        outcome = "continue_sym3_eleven_window_exact_matrix_followup"
        rationale = (
        ...
        next_step = "Keep the Sym^3-lifted eleven-window object only if there is a genuinely different nonlocal family to test. Otherwise pivot to a different higher nonlinear invariant family."

    return Sym3ElevenWindowDecisionGate(
        gate_id="bz_phase2_sym3_eleven_window_decision_gate",
...
def render_sym3_eleven_window_decision_gate() -> str:
    gate = build_sym3_eleven_window_decision_gate()
    lines = [
        "# Phase 2 Sym^3-lifted eleven-window decision gate",
        "",
        f"- Gate id: `{gate.gate_id}`",
        f"- Source probe: `{SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH}`",
...
def write_sym3_eleven_window_decision_gate_report(
    output_path: str | Path = SYM3_ELEVEN_WINDOW_DECISION_GATE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym3_eleven_window_decision_gate(), encoding="utf-8")
    return output


def write_sym3_eleven_window_decision_gate_json(
    output_path: str | Path = SYM3_ELEVEN_WINDOW_DECISION_GATE_JSON_PATH,
) -> Path:
    gate = build_sym3_eleven_window_decision_gate()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

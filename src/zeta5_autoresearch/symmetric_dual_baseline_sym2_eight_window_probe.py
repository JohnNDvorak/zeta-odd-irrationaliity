...
from .dual_packet_schur_functor import build_sym2_lifted_packet_vectors
...
from .symmetric_dual_baseline_sym2_eight_window_object_spec import (
    SYM2_EIGHT_WINDOW_OBJECT_SPEC_REPORT_PATH,
    build_sym2_eight_window_object_spec,
)
from .symmetric_dual_full_packet import build_symmetric_dual_full_packet

SYM2_EIGHT_WINDOW_PROBE_REPORT_PATH = DATA_DIR / "logs" / "bz_phase2_sym2_eight_window_probe.md"
SYM2_EIGHT_WINDOW_PROBE_JSON_PATH = CACHE_DIR / "bz_phase2_sym2_eight_window_probe.json"


Sym2EightWindowVector = tuple[Fraction, ...]


@dataclass(frozen=True)
class Sym2EightWindowSample:
    n: int
...
class Sym2EightWindowProbe:
    ...
    source_boundary: str
    recommendation: str
    samples: tuple[Sym2EightWindowSample, ...]


@lru_cache(maxsize=1)
def build_sym2_eight_window_sequences() -> tuple[tuple[Sym2EightWindowVector, ...], tuple[Sym2EightWindowVector, ...]]:
    ...
    source_lifted = build_sym2_lifted_packet_vectors(source_vectors)
    target_lifted = build_sym2_lifted_packet_vectors(target_vectors)
    source = build_normalized_window_maximal_minor_vectors(source_lifted, window_size=8)
    target = build_normalized_window_maximal_minor_vectors(target_lifted, window_size=8)
    return source, target


def _payload(values: tuple[Sym2EightWindowVector, ...]) -> list[dict[str, object]]:
    return [
...
@lru_cache(maxsize=1)
def build_sym2_eight_window_probe() -> Sym2EightWindowProbe:
    spec = build_sym2_eight_window_object_spec()
    source, target = build_sym2_eight_window_sequences()
    source_payload = _payload(source)
    target_payload = _payload(target)
    source_hash = compute_sequence_hash(
        provisional_signature={"kind": "sym2_eight_window_source", "terms": source_payload}
    )
    target_hash = compute_sequence_hash(
        provisional_signature={"kind": "sym2_eight_window_target", "terms": target_payload}
    )
    paired_hash = compute_sequence_hash(
        provisional_signature={
            "kind": "sym2_eight_window_transfer_object",
    ...
    return Sym2EightWindowProbe(
        probe_id="bz_phase2_sym2_eight_window_probe",
...
def write_sym2_eight_window_probe_report(
    output_path: str | Path = SYM2_EIGHT_WINDOW_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym2_eight_window_probe(), encoding="utf-8")
    return output


def write_sym2_eight_window_probe_json(
    output_path: str | Path = SYM2_EIGHT_WINDOW_PROBE_JSON_PATH,
) -> Path:
    probe = build_sym2_eight_window_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

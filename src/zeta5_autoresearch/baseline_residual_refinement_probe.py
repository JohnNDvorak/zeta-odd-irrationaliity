...
from .baseline_extraction_rule import build_baseline_extraction_rule
from .baseline_residual_refinement_spec import (
    BASELINE_RESIDUAL_REFINEMENT_SPEC_REPORT_PATH,
    BaselineResidualRefinementFamilySpec,
    build_baseline_residual_refinement_spec,
)
...
BASELINE_RESIDUAL_REFINEMENT_PROBE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_baseline_residual_refinement_probe.md"
)
BASELINE_RESIDUAL_REFINEMENT_PROBE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_baseline_residual_refinement_probe.json"
)


@dataclass(frozen=True)
class BaselineResidualRefinementCoefficient:
    label: str
    value: str


@dataclass(frozen=True)
class BaselineResidualSample:
    n: int
    value: str


@dataclass(frozen=True)
class BaselineResidualRefinementFamilyResult:
    family_id: str
    family_label: str
    ...
    coefficients: tuple[BaselineResidualRefinementCoefficient, ...]
    transformed_residual_hash: str
    verdict: str
    zero_count: int
    mismatch_indices: tuple[int, ...]
    first_mismatch_index: int | None
    first_samples: tuple[BaselineResidualSample, ...]


@dataclass(frozen=True)
class BaselineResidualRefinementProbe:
    probe_id: str
    source_spec_id: str
    source_rule_id: str
    ...
    overall_verdict: str
    family_results: tuple[BaselineResidualRefinementFamilyResult, ...]
    bridge_boundary: str
    recommendation: str


def build_baseline_residual_refinement_probe() -> BaselineResidualRefinementProbe:
    spec = build_baseline_residual_refinement_spec()
    rule = build_baseline_extraction_rule()
    constant_terms, zeta3_terms, zeta5_terms = _load_baseline_terms(rule.shared_window_end)
    ...
    overall_verdict = (
        "low_complexity_baseline_refinement_has_winner"
        if any(item.verdict == "holds_on_full_window" for item in family_results)
        else "low_complexity_baseline_refinement_exhausted"
    )
    recommendation = (
        "Promote the winning family into a v2 baseline extraction branch."
        if overall_verdict == "low_complexity_baseline_refinement_has_winner"
        else "Run the residual-refinement decision gate next and stop unless a user-approved pivot is chosen."
    )

    return BaselineResidualRefinementProbe(
        probe_id="bz_phase2_baseline_residual_refinement_probe",
...
def write_baseline_residual_refinement_probe_report(
    output_path: str | Path = BASELINE_RESIDUAL_REFINEMENT_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_baseline_residual_refinement_probe(), encoding="utf-8")
    return output


def write_baseline_residual_refinement_probe_json(
    output_path: str | Path = BASELINE_RESIDUAL_REFINEMENT_PROBE_JSON_PATH,
) -> Path:
    probe = build_baseline_residual_refinement_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def _load_baseline_terms(max_n: int) -> tuple[tuple[Fraction, ...], tuple[Fraction, ...], tuple[Fraction, ...]]:
    ...
    return constant_terms, zeta3_terms, zeta5_terms


def _evaluate_family(
    *,
    family_spec: BaselineResidualRefinementFamilySpec,
    constant_terms: tuple[Fraction, ...],
    zeta3_terms: tuple[Fraction, ...],
    zeta5_terms: tuple[Fraction, ...],
) -> BaselineResidualRefinementFamilyResult:
    ...
    if solution is None:
        coefficients = tuple(
            BaselineResidualRefinementCoefficient(label=label, value="unsolved")
            for label in family_spec.coefficient_labels
        )
        return BaselineResidualRefinementFamilyResult(
            ...
            transformed_residual_hash=compute_sequence_hash(
                provisional_signature={
                    "family_id": family_spec.family_id,
                    "kind": "baseline_residual_refinement_ill_posed",
                    "fit_window_start": family_spec.fit_window_start,
                    "fit_window_end": family_spec.fit_window_end,
                    "validation_window_start": family_spec.validation_window_start,
                    "validation_window_end": family_spec.validation_window_end,
    ...
    transformed_residual_hash = compute_sequence_hash(
        provisional_signature={
            "kind": "baseline_residual_refinement_family",
            "family_id": family_spec.family_id,
            "fit_window_start": family_spec.fit_window_start,
            "fit_window_end": family_spec.fit_window_end,
            "validation_window_start": family_spec.validation_window_start,
    ...
    coefficients = tuple(
        BaselineResidualRefinementCoefficient(
            label=label,
    ...
    verdict = (
        "holds_on_full_window" if not mismatch_indices else "fails_after_fit_window"
    )
    return BaselineResidualRefinementFamilyResult(
        family_id=family_spec.family_id,
        family_label=family_spec.family_label,
    ...
    *,
    ...
    for n in range(family_spec.fit_window_start, family_spec.fit_window_end + 1):
        if family_spec.family_id == "support0_same_index":
            ...
            rows.append(
                (
                    constant_terms[n - 1] - constant_terms[n - 2],
            ...
            raise ValueError(f"unsupported residual-refinement family {family_spec.family_id!r}")
    return tuple(rows)
...
def _compute_transformed_residual(
    *,
    ...
    if family_id == "support0_same_index":
        a, b = coefficients
        return zeta3_terms[n - 1] + a * constant_terms[n - 1] + b * zeta5_terms[n - 1]
    if family_id == "difference_pair":
        a, b = coefficients
        return zeta3_terms[n - 1] + a * (constant_terms[n - 1] - constant_terms[n - 2]) + b * (
            zeta5_terms[n - 1] - zeta5_terms[n - 2]
        )
    ...
    raise ValueError(f"unsupported residual-refinement family {family_id!r}")

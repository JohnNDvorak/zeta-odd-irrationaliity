...
from .symmetric_dual_baseline_eight_window_plucker_object_spec import (
...
EIGHT_WINDOW_NORMALIZED_PLUCKER_PROBE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_eight_window_normalized_plucker_probe.md"
)
EIGHT_WINDOW_NORMALIZED_PLUCKER_PROBE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_eight_window_normalized_plucker_probe.json"
)
...
class EightWindowNormalizedPluckerSample:
    n: int
...
@dataclass(frozen=True)
class EightWindowNormalizedPluckerProbe:
    probe_id: str
    source_spec_id: str
    ...
    source_boundary: str
    recommendation: str
    samples: tuple[EightWindowNormalizedPluckerSample, ...]


def build_eight_window_normalized_plucker_sequences() -> tuple[tuple[PluckerVector, ...], tuple[PluckerVector, ...]]:
    ...
    source = build_normalized_window_plucker_vectors(source_vectors, window_size=8)
    target = build_normalized_window_plucker_vectors(target_vectors, window_size=8)
...
def build_eight_window_normalized_plucker_probe() -> EightWindowNormalizedPluckerProbe:
    ...
    source_hash = compute_sequence_hash(
        provisional_signature={"kind": "eight_window_plucker_source", "terms": source_payload}
    )
    target_hash = compute_sequence_hash(
        provisional_signature={"kind": "eight_window_plucker_target", "terms": target_payload}
    )
    paired_hash = compute_sequence_hash(
        provisional_signature={
            "kind": "eight_window_plucker_transfer_object",
    ...
    samples = tuple(
        EightWindowNormalizedPluckerSample(
            n=n,
    ...
    return EightWindowNormalizedPluckerProbe(
        probe_id="bz_phase2_eight_window_normalized_plucker_probe",
        ...
        verdict="eight_window_normalized_plucker_object_established",
        stabilized_findings=(
            "The symmetric-dual and baseline-dual eight-window normalized Plucker invariants are now repo-native exact objects.",
            "The eight-window normalized invariant has coordinate count `55` on the shared exact window `n=1..73`.",
            "Source, target, and paired nonlinear objects are independently reproducible via exact hashes.",
        ),
        unresolved_findings=(
            "No recurrence-level transfer family has yet been certified on this eight-window object.",
        ...
        recommendation=(
            "Run only the overdetermined order-1 constant matrix recurrence screen on source and target separately before choosing another family."
        ),
        samples=samples,
    )


def render_eight_window_normalized_plucker_probe() -> str:
    probe = build_eight_window_normalized_plucker_probe()
    lines = [
        "# Phase 2 eight-window normalized Plucker probe",
        "",
        f"- Probe id: `{probe.probe_id}`",
        f"- Source spec: `{EIGHT_WINDOW_NORMALIZED_PLUCKER_OBJECT_SPEC_REPORT_PATH}`",
...
def write_eight_window_normalized_plucker_probe_report(
    output_path: str | Path = EIGHT_WINDOW_NORMALIZED_PLUCKER_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_eight_window_normalized_plucker_probe(), encoding="utf-8")
    return output


def write_eight_window_normalized_plucker_probe_json(
    output_path: str | Path = EIGHT_WINDOW_NORMALIZED_PLUCKER_PROBE_JSON_PATH,
) -> Path:
    probe = build_eight_window_normalized_plucker_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

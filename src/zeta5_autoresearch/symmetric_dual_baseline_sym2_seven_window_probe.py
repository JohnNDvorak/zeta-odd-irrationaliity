...
from .symmetric_dual_baseline_sym2_seven_window_object_spec import (
...
SYM2_SEVEN_WINDOW_PROBE_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym2_seven_window_probe.md"
)
SYM2_SEVEN_WINDOW_PROBE_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym2_seven_window_probe.json"
)


InvariantVector = tuple[Fraction, ...]


@dataclass(frozen=True)
class Sym2SevenWindowSample:
    n: int
...
class Sym2SevenWindowProbe:
    ...
    source_boundary: str
    recommendation: str
    samples: tuple[Sym2SevenWindowSample, ...]


@lru_cache(maxsize=1)
def build_sym2_seven_window_sequences() -> tuple[tuple[InvariantVector, ...], tuple[InvariantVector, ...]]:
    ...
    source = build_normalized_window_maximal_minor_vectors(source_lifted, window_size=7)
    target = build_normalized_window_maximal_minor_vectors(target_lifted, window_size=7)
    return source, target


def _payload(values: tuple[InvariantVector, ...]) -> list[dict[str, object]]:
    return [
        {
            "n": n,
...
@lru_cache(maxsize=1)
def build_sym2_seven_window_probe() -> Sym2SevenWindowProbe:
    ...
    source_hash = compute_sequence_hash(
        provisional_signature={"kind": "sym2_seven_window_source", "terms": source_payload}
    )
    target_hash = compute_sequence_hash(
        provisional_signature={"kind": "sym2_seven_window_target", "terms": target_payload}
    )
    paired_hash = compute_sequence_hash(
        provisional_signature={
            "kind": "sym2_seven_window_transfer_object",
    ...
    return Sym2SevenWindowProbe(
        probe_id="bz_phase2_sym2_seven_window_probe",
...
def write_sym2_seven_window_probe_report(
    output_path: str | Path = SYM2_SEVEN_WINDOW_PROBE_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym2_seven_window_probe(), encoding="utf-8")
    return output


def write_sym2_seven_window_probe_json(
    output_path: str | Path = SYM2_SEVEN_WINDOW_PROBE_JSON_PATH,
) -> Path:
    probe = build_sym2_seven_window_probe()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

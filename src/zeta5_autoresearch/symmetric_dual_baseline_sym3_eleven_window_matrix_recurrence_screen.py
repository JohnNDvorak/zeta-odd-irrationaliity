...
SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym3_eleven_window_matrix_recurrence_screen.md"
)
SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym3_eleven_window_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class Sym3ElevenWindowMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class Sym3ElevenWindowMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[Sym3ElevenWindowMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> Sym3ElevenWindowMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return Sym3ElevenWindowMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_sym3_eleven_window_matrix_recurrence_screen() -> Sym3ElevenWindowMatrixRecurrenceScreen:
    ...
    return Sym3ElevenWindowMatrixRecurrenceScreen(
        screen_id="bz_phase2_sym3_eleven_window_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "sym3_eleven_window_matrix_recurrence_requires_exact_followup"
            if any_open
            else "low_order_matrix_recurrence_exhausted_through_order_6"
        ),
        ...
        recommendation=(
            ...
            if any_open
            else "Do not keep increasing matrix recurrence order mechanically. Order 7 is the first underdetermined case on this lifted object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_sym3_eleven_window_matrix_recurrence_screen() -> str:
    screen = build_sym3_eleven_window_matrix_recurrence_screen()
    lines = [
        "# Phase 2 Sym^3-lifted eleven-window matrix recurrence screen",
        "",
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "This screen tests constant matrix-valued recurrences on the Sym^3-lifted eleven-window normalized maximal-minor sequence, for recurrence orders `1` through `6`, separately on the source and target side. Order `7` is the first underdetermined case on this lifted object.",
            "",
            "## Source boundary",
            "",
...
def write_sym3_eleven_window_matrix_recurrence_screen_report(
    output_path: str | Path = SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym3_eleven_window_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_sym3_eleven_window_matrix_recurrence_screen_json(
    output_path: str | Path = SYM3_ELEVEN_WINDOW_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_sym3_eleven_window_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

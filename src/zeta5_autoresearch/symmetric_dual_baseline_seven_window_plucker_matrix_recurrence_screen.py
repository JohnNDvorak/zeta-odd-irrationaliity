...
SEVEN_WINDOW_NORMALIZED_PLUCKER_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_seven_window_normalized_plucker_matrix_recurrence_screen.md"
)
SEVEN_WINDOW_NORMALIZED_PLUCKER_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_seven_window_normalized_plucker_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class SevenWindowNormalizedPluckerMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class SevenWindowNormalizedPluckerMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[SevenWindowNormalizedPluckerMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> SevenWindowNormalizedPluckerMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return SevenWindowNormalizedPluckerMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_seven_window_normalized_plucker_matrix_recurrence_screen() -> (
    SevenWindowNormalizedPluckerMatrixRecurrenceScreen
):
    ...
    return SevenWindowNormalizedPluckerMatrixRecurrenceScreen(
        screen_id="bz_phase2_seven_window_normalized_plucker_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "seven_window_matrix_recurrence_requires_exact_followup"
            if any_open
            else "low_order_matrix_recurrence_exhausted_through_order_2"
        ),
        source_boundary=probe.source_boundary,
        recommendation=(
            "A modular obstruction was not found on at least one side/order; exact follow-up is required before banking the seven-window matrix ladder as exhausted."
            if any_open
            else "Do not keep increasing matrix recurrence order mechanically. Order 3 is the first underdetermined case on this object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_seven_window_normalized_plucker_matrix_recurrence_screen() -> str:
...
def write_seven_window_normalized_plucker_matrix_recurrence_screen_report(
    output_path: str | Path = SEVEN_WINDOW_NORMALIZED_PLUCKER_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_seven_window_normalized_plucker_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_seven_window_normalized_plucker_matrix_recurrence_screen_json(
    output_path: str | Path = SEVEN_WINDOW_NORMALIZED_PLUCKER_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_seven_window_normalized_plucker_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

...
SYM4_SIXTEEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH = (
    DATA_DIR / "logs" / "bz_phase2_sym4_sixteen_window_matrix_recurrence_screen.md"
)
SYM4_SIXTEEN_WINDOW_MATRIX_RECURRENCE_SCREEN_JSON_PATH = (
    CACHE_DIR / "bz_phase2_sym4_sixteen_window_matrix_recurrence_screen.json"
)


@dataclass(frozen=True)
class Sym4SixteenWindowMatrixRecurrenceResult:
    packet_side: str
...
@dataclass(frozen=True)
class Sym4SixteenWindowMatrixRecurrenceScreen:
    screen_id: str
    source_probe_id: str
    shared_window_start: int
    shared_window_end: int
    order_results: tuple[Sym4SixteenWindowMatrixRecurrenceResult, ...]
    overall_verdict: str
    source_boundary: str
    recommendation: str
...
def _screen_matrix_recurrence(
    *,
    ...
    vectors: tuple[tuple[Fraction, ...], ...],
    recurrence_order: int,
) -> Sym4SixteenWindowMatrixRecurrenceResult:
    dimension = len(vectors[0])
    ...
    for prime in _matrix_recurrence_witness_primes():
        ...
        if inconsistent is None:
            continue
        if inconsistent:
            return Sym4SixteenWindowMatrixRecurrenceResult(
...
@lru_cache(maxsize=1)
def build_sym4_sixteen_window_matrix_recurrence_screen() -> Sym4SixteenWindowMatrixRecurrenceScreen:
    probe = build_sym4_sixteen_window_probe()
    source, target = build_sym4_sixteen_window_sequences()

    results = []
    for recurrence_order in range(1, 5):
        results.append(
    ...
    return Sym4SixteenWindowMatrixRecurrenceScreen(
        screen_id="bz_phase2_sym4_sixteen_window_matrix_recurrence_screen",
        ...
        overall_verdict=(
            "sym4_sixteen_window_matrix_recurrence_requires_exact_followup"
            if any_open
            else "low_order_matrix_recurrence_exhausted_through_order_4"
        ),
        ...
        recommendation=(
            ...
            if any_open
            else "Do not keep increasing matrix recurrence order mechanically. Order 5 is the first underdetermined case on this lifted object, so further escalation would cross from obstruction screening into interpolation territory."
        ),
    )


def render_sym4_sixteen_window_matrix_recurrence_screen() -> str:
    screen = build_sym4_sixteen_window_matrix_recurrence_screen()
    lines = [
...
def write_sym4_sixteen_window_matrix_recurrence_screen_report(
    output_path: str | Path = SYM4_SIXTEEN_WINDOW_MATRIX_RECURRENCE_SCREEN_REPORT_PATH,
) -> Path:
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_sym4_sixteen_window_matrix_recurrence_screen(), encoding="utf-8")
    return output


def write_sym4_sixteen_window_matrix_recurrence_screen_json(
    output_path: str | Path = SYM4_SIXTEEN_WINDOW_MATRIX_RECURRENCE_SCREEN_JSON_PATH,
) -> Path:
    screen = build_sym4_sixteen_window_matrix_recurrence_screen()
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    write_sym4_sixteen_window_matrix_recurrence_screen_report()
    write_sym4_sixteen_window_matrix_recurrence_screen_json()
